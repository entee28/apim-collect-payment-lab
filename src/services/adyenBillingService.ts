...
export class AdyenBillingService implements BillingService {
  public async unsubscribe(_apimSubscriptionId: string) {
    /*
    ...
    await Promise.all(
      subscriptions
        ...
        .map(async (sub) => {
          ...
          const userId = sub.ownerId && sub.ownerId.replace("/users/", "");
          ...
          if (product && subscriptionId && userId) {
  ...
  public static async takePaymentFromUser(invoice: Invoice) {
    ...
    if (invoice.amount > 0) {
      ...
      try {
        ...
        // console.log(
        //   "THE PAYMENT METHODS RESPONSE",
        //   paymentMethodsResponse.paymentMethods
        // );

        if (paymentMethodsResponse.storedPaymentMethods) {
          console.log("THE IF PAYMENT METHODS RESPONSE RAN");
          ...
          if (
            ...
            console.log("THE IF PAYMENT ERROR CODE RAN");
          ...
          // Command the below code block to prevent subscirption deactivation for testing purpose
